...
func init() {
	android.RegisterModuleType("pac_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = pacExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = pacExpandVariables(ctx, dep_file)
	...
	cmd := pacExpandVariables(ctx, String(g.properties.Cmd))
